...
class Encoder(nn.Module):
    ...
    def forward(self, x_pv, x_art):
        ...
        return aggr_scale_2, aggr_scale_3, aggr_scale_4, aggr_scale_5
...
class Multi_Phase_Lits(nn.Module):
    def __init__(self, in_ch, out_ch, num_classes=2):
        super().__init__()
        self.encoder = Encoder()
        self.head = Seg_head(num_classes)
        self.urim = URIM()
        # self.map_conv_1 = nn.Conv2d(in_channels=2, out_channels=2, kernel_size=5, padding=2)
    def forward(self, x_pv, x_art, pv_mask, art_mask):
        size = x_pv.size()[2:]
        ...
        predicts = initial_seg, final_seg
        metric_imgs = F.softmax(final_seg, dim=1)

        # predicts, targets, metric_imgs, metric_targets
        return predicts, pv_mask, metric_imgs[:,1].unsqueeze(1), pv_mask
...
if __name__ == '__main__':
    ...
    a = torch.ones((2, 3, 512, 512)).to(device)
    b = torch.ones((2, 3, 512, 512)).to(device)
    c = torch.ones((2, 1, 256, 256)).to(device)
    d = torch.ones((2, 1, 256, 256)).to(device)

    model = Multi_Phase_Lits(num_classes=2).to(device)
    predicts, targets, metric_imgs, metric_targets = model(a,b,c,d)
    # print(predicts.size())
    # print(targets.size())
    # print(metric_imgs.size())
    # print(metric_targets.size())