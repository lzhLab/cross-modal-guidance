...
class _CMG(nn.Module):
    def __init__(self, in_ch, out_ch):
        super().__init__()

        self.pv_conv1_1 = nn.Conv2d(in_ch, out_ch, 1, padding=0, bias=False)
        self.pv_conv1_2 = nn.Conv2d(out_ch, out_ch // 2, 1, padding=0, bias=False)
        self.art_conv1_1 = nn.Conv2d(in_ch, out_ch, 1, padding=0, bias=False)
        self.art_conv1_2 = nn.Conv2d(out_ch, out_ch // 2, 1, padding=0, bias=False)
    ...
    def forward(self, pv_merge, art_merge, pv_skip, art_skip, pv_mask, art_mask):
        ...
        if self.training:
            ...
            pv_pred_mask = F.adaptive_max_pool2d(pv_mask, output_size=mask_W)
            art_pred_mask = F.adaptive_max_pool2d(art_mask, output_size=mask_W)
        else:
            pv_pred_mask = pv_pred > 0
            art_pred_mask = art_pred > 0

        # masked average pooling
        pv_skip_masked = pv_skip * pv_pred_mask
        ...
        pv_pred_mask = pv_pred_mask.view(B, -1)
        art_pred_mask = art_pred_mask.view(B, -1)


        pv_vector = torch.sum(pv_skip_masked, dim=2) / (1e-4 + torch.sum(pv_pred_mask, dim=1)).unsqueeze(dim=1).expand([B, C])
        ...
        return pv_feature, art_feature, pv_pred, art_pred, pv_similarity, art_similarity


class Cross_Modal_Guidance(nn.Module):
    ...
    def __init__(self, in_ch, out_ch, **args):
        ...
        # fusion-block
        self.CMG1 = _CMG(self.scale * 16 + self.scale * 8, self.scale * 8)
        self.CMG2 = _CMG(self.scale * 8 * 2 + self.scale * 4, self.scale * 4)
    ...
    def forward(self, pv_imgs, art_imgs, pv_mask, art_mask):
        ...
        pv_co1, art_co1, pv_pred_2, art_pred_2, pv_simi_1, art_simi_1 = self.CMG1(pv_merge6, art_merge6, pv_c4, art_c4, pv_mask, art_mask)
        ...
        pv_co2, art_co2, pv_pred_3, art_pred_3, pv_simi_2, art_simi_2 = self.CMG2(pv_merge7, art_merge7, pv_c3, art_c3, pv_mask, art_mask)
