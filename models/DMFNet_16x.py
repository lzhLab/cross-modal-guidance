...
class MFNet(nn.Module): #
    ...
    def __init__(self, c=2, n=32,channels=128,groups = 16,norm='bn', num_classes=5):
    ...
    def forward(self, t1_img, t2_img, t1_mask, t2_mask):
        ...
        x = torch.cat([t1_img, t2_img], dim=1)
        ...
        return y4, t1_mask.squeeze(1).long(), y4, t1_mask


class DMFNet(MFNet): # softmax
    # [128]  Flops:  27.045G  &  Params: 3.88M
    def __init__(self, in_ch, out_ch ,n=32,channels=128, groups=16,norm='sync_bn'):

        super(DMFNet, self).__init__(2,n,channels,groups, norm, out_ch)
...
if __name__ == '__main__':
    ...
    # device = torch.device('cpu')
    x1 = torch.rand((1,1,16,128,128),device=device) # [bsize,channels,Height,Width,Depth]
    x2 = torch.rand((1,1,16,128,128),device=device) # [bsize,channels,Height,Width,Depth]
    x3 = torch.rand((1,5,16,128,128),device=device) # [bsize,channels,Height,Width,Depth]
    x4 = torch.rand((1,5,16,128,128),device=device) # [bsize,channels,Height,Width,Depth]
    model = DMFNet(2, 5)
    # model.cuda(device)
    y4, t1_mask, y4, t1_mask = model(x1, x2,x3,x4)
    print(y4.shape)
    print(t1_mask.shape)